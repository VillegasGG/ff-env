...
def generate_positions(n_nodes, edges, space_limits):
    """
    ...
    limit_min, limit_max = space_limits
    # Ajusta el centro y la escala de las posiciones
    center = [(limit_max + limit_min) / 2] * 3
    scale = (limit_max - limit_min) / 2
    # Utiliza el layout de Fruchterman-Reingold para generar posiciones
    return nx.drawing.layout.fruchterman_reingold_layout(tree, dim=3, scale=scale, center=center)
...
def create_tree_from_sequence(sequence, space_limits, add_positions=True):
    """
    ...
    if add_positions:
        positions = generate_positions(n_nodes, edges, space_limits)
...
def generate_random_tree(n_nodes, root_degree, type_root_degree, space_limits, add_positions=True, max_trials=1000):    
    """
...
def generate_tree_from_adjacency_matrix(adjacency_matrix, positions):
    """
    ...
    if edges.shape[0] == 0:
        raise ValueError("The adjacency matrix does not represent a valid tree.")
    
    edges_list = [(edge[0], edge[1]) for edge in edges]
    
    return Tree(np.arange(n_nodes), np.array(edges_list), positions)
