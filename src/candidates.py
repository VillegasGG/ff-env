...
def get_distances_to_nodes(nodes, tree, ff):
    """
    Calculate the distances from the firefighter's position to each node in the tree.
    """
    distances = {}
    for node in nodes:
        distances[int(node)] = float(np.linalg.norm(tree.nodes_positions[node] - ff.position))
    return distances
...
def get_candidates(tree, state, ff):
    ...
    ff_distances = get_distances_to_nodes(first_candidates, tree, ff)
